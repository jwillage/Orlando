...
```{r include = F}
o.prior <- readRDS("rds/o.prior.rds")
```
...
`r round(mean(table(hour(created.prior))[-c(3:5)]), 0)` tweets per hour, and 
`r round(mean(table(paste(hour(created.prior), minute(created.prior)))), 1)` tweets per minute (tpm). 
...
The 1,000 tweets *prior* to the attack occurred in a span of `r round(created.prior[1] - created.prior[1000], 1)` hours, whereas *post*-attack, 1,000 tweets racked up in `r round(created.post[1] - created.post[1000], 1)` hours. 
...
```{r, include = F} 
users.313 <- readRDS("rds/users.313.rds")
```
...
All of the analysis so far was on the 1,000 #orlando tweets following 2:09 am, a span of `r round(created.post[1] - created.post[1000], 1)` hours. Now we expand to look at the 10,000 tweets mentioning #orlando after 3/12 2:09 am. 
...
10,000 #orlando tweets after 2:09 am are logged in a span of `r round(created[1] - created[9864], 1)` hours. Now we have a period up to 5:25. Most of the tpm increase appears to happen between 3:35 and 3:55, as highlighted in the graph. Again, we use the rate of tpm compared to the previous minute. 
...
The only time in the steep curve (and through the rest of the data) when the rate increases by at least 50% is at 3:45, with an increase from 42 to 64 tpm. We still see increases at a less steep rate through 3:55, where it plateaus at about `r round(mean(times[70:nrow(times), "tweets"]),0)` tpm. Exploring that jump at 3:45 we see the following...
...
```{r, include = F} 
users.345 <- readRDS("rds/users.345.rds")
```
...
```{r, include = F} 
users.349 <- readRDS("rds/users.349.rds")
```
